...
class WriterConfigurationTest(
    ...
    fun `should save clients`() {
        val (client1, client2) = dummyClients()
    ...
    @Test
    @Sql("/client.sql")
    fun `should update client given existing client id`() {
        val (client1, client2) = dummyClients()
        val existingClient = repository.findByIdOrNull(1)?.copy()
        ...
        assertThat(count).isEqualTo(2)
        val actualClient1 = repository.findByIdOrNull(1)
        assertThat(actualClient1).isNotEqualTo(existingClient)
        assertThat(actualClient1).isEqualTo(client1)
        val actualClient2 = repository.findByIdOrNull(2)
        assertThat(actualClient2).isEqualTo(client2)
    }

    private fun dummyClients() =
        Client(
            id = 1,
